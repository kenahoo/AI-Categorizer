...
sub add_hypothesis {
  ...
  # Add to the macro tables
  {
    ...
    $this{c} = keys(%$correct) - $this{a};
    $this{d} = keys(%$cats_table) - $this{a} - $this{b} - $this{c};
  ...
  # Add to the micro tables
  foreach my $cat (keys %$cats_table) {
    $cats_table->{$cat}{a}++ if  $assigned{$cat} and  $correct->{$cat};
    $cats_table->{$cat}{b}++ if  $assigned{$cat} and !$correct->{$cat};
    $cats_table->{$cat}{c}++ if !$assigned{$cat} and  $correct->{$cat};
    $cats_table->{$cat}{d}++ if !$assigned{$cat} and !$correct->{$cat};
  }
